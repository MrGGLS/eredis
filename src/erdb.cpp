...
bool save_data(ERedisServer* server, std::string base_path){
    ...
    for(auto i=0;i<server->db_num;i++){
        ...
        /* write db size */
        int edb_size = edb->dict.size();
        writer.write((char*)&edb_size,4);
        time_t expire_ms;
        int str_len;
        int list_len;
        uint8_t data_type;
