...
//
#include "eredis.hpp"
ERedisDb::ERedisDb(int id):id(id) {}
ERedisServer::ERedisServer(std::vector<ERedisDb*> _db, int _db_num):db(_db),db_num(_db_num){}
...
std::string ERedisServer::getrange(int db_id, std::string key, int start, int end) {
    ERedisDb* edb = this->db[db_id];
    if(edb->expires.count(key)&&edb->expires[key]<time(0))
        del_key(db_id,key);
    if(edb->dict.count(key)){
...
ERedisServer getInstanceOfServer(int db_num){
